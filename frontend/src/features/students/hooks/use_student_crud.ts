...
type FormState = { name: string; email: string; phone: string; client_id: string };
const emptyForm: FormState = { name: "", email: "", phone: "", client_id: "" };
...
export function useStudentCrud(refresh: () => Promise<void>) {
    ...
    function openEdit(stu: Student) {
        setEditing(stu);
        setForm({ name: stu.person.name, email: stu.person.email ?? "", phone: stu.person.phone ?? "", client_id: stu.client_id ?? "" });
        setShowModal(true);
    }
