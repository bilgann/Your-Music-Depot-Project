...
def parse_db_error(exc: Exception) -> Exception:
    """
    Inspect a raw Supabase / PostgreSQL exceptions and return a typed
    application exceptions, or the original if the cause is unknown.
    """
    msg = str(exc).lower()
...
def error_response(exc: Exception):
    """
    Convert any exceptions to a standardized (flask.Response, status_code) tuple.
